...
std::string decryptKey_linux(std::string const &secret, std::string const &encryptedkeystr)
{
  ...
  if (PKCS5_PBKDF2_HMAC_SHA1(reinterpret_cast<char const *>(secret.data()), secret.size(), salt, salt_length, 1, key_length, key.get()) != 1)
  {
